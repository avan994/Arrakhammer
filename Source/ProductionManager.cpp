...
void ProductionManager::update() 
{
	...
	if (_targetGasAmount != 0 && BWAPI::Broodwar->self()->gatheredGas() >= _targetGasAmount)  // tends to go over
	{
...
void ProductionManager::manageBuildOrderQueue() 
{
	...
	//check if we have a gas problem -- start collecting gas if so
	if (getFreeGas() < 0){
		WorkerManager::Instance().setCollectGas(true, true);
		auto r = BWAPI::Broodwar->self()->getRace().getRefinery();
		if (UnitUtil::GetAllUnitCount(r) < 1 && !BuildingManager::Instance().isBeingBuilt(r) && InformationManager::Instance().getMyNumGeysers() > 0)
			_queue.queueAsHighestPriority(r);
	}
	...
	while (!_queue.isEmpty()) 
	{
		...
		BWAPI::Unit producer;
		if (currentItem.macroAct.type() == MacroActs::Unit && 
			(currentItem.macroAct.getUnitType() == BWAPI::UnitTypes::Zerg_Lair || 
			currentItem.macroAct.getUnitType() == BWAPI::UnitTypes::Zerg_Lurker)) {
			producer = getProducer(currentItem.macroAct, InformationManager::Instance().getMyMainBaseLocation()->getPosition());
		}
		else {
			producer = getProducer(currentItem.macroAct);
		}
...
BWAPI::Unit ProductionManager::getProducer(MacroAct t, BWAPI::Position closestTo)
{
	...
	// Prefer selecting larva close to the best depot
	if (t.isUnit() && t.getUnitType().isWorker())
	{
		BWAPI::Unit bestDepot = WorkerManager::Instance().getBestDepot();
		if (bestDepot) { //whatever pops up first as being unsaturated. 
			return getClosestUnitToPosition(candidateProducers, bestDepot->getPosition());
		}
		else { //no depots?!?!
	...
	else if (t.whatBuilds() == BWAPI::UnitTypes::Zerg_Larva && closestTo == BWAPI::Positions::None) {
		BWAPI::Unit bestDepot = getBestHatchery();
		if (bestDepot) {
			if (bestDepot->getLarva().size() == 0) {
				UAB_ASSERT(false, "Critical error: Selected hatchery with 0 larva!");
				return getClosestUnitToPosition(candidateProducers, closestTo);
			}
			return *(bestDepot->getLarva().begin());
		}
		else { //no hatcheries with larva or anything, just do whatever
			return getClosestUnitToPosition(candidateProducers, closestTo);
		}
	}
	else
...
//
BWAPI::Unit ProductionManager::getBestHatchery() {
	...
	for (auto & unit : BWAPI::Broodwar->self()->getUnits())
	{
		...
		if (type.getRace() != BWAPI::Races::Zerg) continue;
...
void ProductionManager::create(BWAPI::Unit producer, BuildOrderItem & item) 
{
		...
		&& !act.getUnitType().isAddon())                        // terran addon
	{
		...
		else if (act.getMacroLocation() == MacroLocation::LeastDefended) {
			BWTA::BaseLocation * leastDefended = InformationManager::Instance().getMyLeastDefendedLocation();
			if (leastDefended)
			{
				desiredLocation = leastDefended->getTilePosition();
			}
		}
	...
	{
		if (act.getUnitType().getRace() == BWAPI::Races::Zerg && act.getUnitType() != BWAPI::UnitTypes::Zerg_Infested_Terran)
		{
...
void ProductionManager::drawProductionInformation(int x, int y)
{
	...
	std::vector<BWAPI::Unit> research;
	for (auto & unit : BWAPI::Broodwar->self()->getUnits())
	{
        UAB_ASSERT(unit != nullptr, "Unit was null");
		...
		else if (unit->isResearching() && unit->getTech() != BWAPI::TechTypes::None) {
			y += 10;
			
			BWAPI::Broodwar->drawTextScreen(x, y, " %c%s", red, unit->getTech().c_str());
			BWAPI::Broodwar->drawTextScreen(x - 35, y, "%c%6d", green, unit->getRemainingResearchTime());
		}
		else if (unit->isUpgrading() && unit->getUpgrade() != BWAPI::UpgradeTypes::None) {
			y += 10;
			BWAPI::Broodwar->drawTextScreen(x, y, " %c%s", red, unit->getUpgrade().c_str());
			BWAPI::Broodwar->drawTextScreen(x - 35, y, "%c%6d", green, unit->getRemainingUpgradeTime());
		}
		else if (unit->getType() == BWAPI::UnitTypes::Zerg_Evolution_Chamber) {
			y += 10;
			BWAPI::Broodwar->drawTextScreen(x, y, " %c%s", white, "Idle: Evolution Chamber");
		}
	}
...
void ProductionManager::startExtractorTrick()
{
	...
	// If we're not supply blocked, then we may have lost units earlier. No need to do the trick. 
	if (BWAPI::Broodwar->self()->supplyTotal() - BWAPI::Broodwar->self()->supplyUsed() >= 2)
	{
		//_queue.queueAsHighestPriority(MacroAct(BWAPI::UnitTypes::Zerg_Drone)); //this line causes a crash
		return;
	}
...
void ProductionManager::doExtractorTrick()
{
	...
	else if (_extractorTrickState == ExtractorTrick::ExtractorOrdered)
	{
		...
		else if (supplyAvail < -2) { // Uh oh, we must have lost an overlord or a hatchery. Give up by moving on. 
			_extractorTrickState = ExtractorTrick::DroneOrdered;
		}
		else if (WorkerManager::Instance().getNumMineralWorkers() <= 0) { // Yow, there must have been a drone massacre. Give up by moving on.
			_extractorTrickState = ExtractorTrick::DroneOrdered;
		}
	}
	else if (_extractorTrickState == ExtractorTrick::DroneOrdered)
...
bool ProductionManager::nextIsBuilding()
{
	if (_queue.isEmpty())
	{
		return false;
