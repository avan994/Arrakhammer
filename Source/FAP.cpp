...
namespace UAlbertaBot {
		...
		player(ui.player)
	{
		...
		if (ui.type == BWAPI::UnitTypes::Protoss_Carrier) {
			...
			//assume there'll be an SCV repairing it
			//overcompensate for the bunker's strength against ground units
			health += 100;
			groundMaxRange += 32;
		}
			...
			//increase their range a bit more for ground units
			groundMaxRange += 32;
		}
		else if (ui.type == BWAPI::UnitTypes::Protoss_Dark_Templar) {
			//just for dark templar for now
			if (ui.unit && ui.unit->exists() &&
				(ui.unit->isCloaked() || ui.unit->isBurrowed()) && !(ui.unit->isDetected())) { //temp fix for cloaked units
				health += 500;
			}
		}
